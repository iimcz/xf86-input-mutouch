  ...
  while (num_bytes >= (MuT_REPORT_SIZE - bytes_in_packet))
  {
    ...
    if (bytes_in_packet == 5)
    {
      ...
      DBG(3, ErrorF("TouchScreen %s: x(%d), y(%d), %s\n",
                    ((state & MuT_WHICH_DEVICE) ? "Stylus" : "Finger"),
                    cur_x, cur_y,
                    (((state & MuT_CONTACT) != (priv->state & MuT_CONTACT)) ? ((state & MuT_CONTACT) ? "Press" : "Release") : "Stream")));
      priv->state = state;
    }
  }
               ...
               int mode)
{
  ...
  case DEVICE_ON:
  {
    ...
    if (!already_open)
    {
      ...
      /*
       * Set the operating mode: Format Tablet, Mode stream, Finger.
       */
      memset(req, 0, MuT_PACKET_SIZE);
      ...
      memset(req, 0, MuT_PACKET_SIZE);
      strncpy((char *)&req[1], MuT_FINGER_ONLY, strlen(MuT_FINGER_ONLY));
      if (xf86MuTSendCommand(req, strlen(MuT_FINGER_ONLY),
                             NULL, pInfo->fd) != Success)
      {
    ...
    if (xf86MuTSendCommand(req, strlen(report_what), NULL, pInfo->fd) != Success)
    {
      ErrorF("Unable to change MuTouch touchscreen to %s\n",
             (strcmp(report_what, MuT_PEN_FINGER) == 0) ? "Pen & Finger" : ((strcmp(report_what, MuT_PEN_ONLY) == 0) ? "Pen Only" : "Finger Only"));
      goto not_success;
    }
    dev->public.on = TRUE;
    /*
